...
BEGIN
  ...
  CREATE OR REPLACE TABLE `lakehouse-486419.curated_dev.sample_curated`
  OPTIONS (
    description = "Curated table built from external Parquet (BigLake).",
    labels = [("layer","curated"), ("env","dev")]
  ) AS
  SELECT
  ...
  ASSERT (SELECT COUNT(*) FROM `lakehouse-486419.curated_dev.sample_curated`) > 0
    AS "CURATED sample_curated is empty";

  ASSERT (SELECT COUNT(*) FROM `lakehouse-486419.curated_dev.sample_curated` WHERE id IS NULL) = 0
