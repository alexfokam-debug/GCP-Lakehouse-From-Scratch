...
############################################################
# Secret Manager IAM for Dataform Git Token (Enterprise)
#
# But :
# - Donner à Dataform le droit de lire le secret "dataform-git-token"
# - Nettoyer les bindings pollués (ex: deleted:serviceAccount:...)
# - Garder le token hors de Terraform state
############################################################
...
# Le secret existe déjà (créé manuellement, ou par un autre process)
data "google_secret_manager_secret" "dataform_git_token" {
  project   = var.project_id
  secret_id = var.dataform_git_token_secret_id
}

resource "google_secret_manager_secret_iam_binding" "dataform_git_token_accessor" {
  project   = var.project_id
  secret_id = data.google_secret_manager_secret.dataform_git_token.secret_id
  ...
  ##########################################################
  # AUTHORITATIVE:
  # - cette liste devient LA vérité
  # - si tu avais des "deleted:..." dans l'IAM => ils disparaissent
  ##########################################################
  members = [
    # Service Agent Dataform (OBLIGATOIRE)
    "serviceAccount:service-${data.google_project.current.number}@gcp-sa-dataform.iam.gserviceaccount.com",

    # Ton SA Dataform runtime (utile + souvent nécessaire selon usage)
    "serviceAccount:${var.dataform_sa_email}",
  ]
}